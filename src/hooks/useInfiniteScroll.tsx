...
export function useInfiniteScroll<T>(
  ...
  const { data, loading } = useCollection(
    query(collection, ...[...(queryConstraints ?? []), ...(!reachedLimit ? [limit(tweetsLimit)] : [])]),
    fetchOptions
  )
  ...
  useEffect(() => {
    ...
    const setTweetsLength = async (): Promise<void> => {
      const currentTweetsSize = await getCollectionCount(query(collection, ...(queryConstraints ?? [])))
  ...
  const isLoadMoreHidden = reachedLimit && (data?.length ?? 0) >= (tweetsSize ?? 0)
