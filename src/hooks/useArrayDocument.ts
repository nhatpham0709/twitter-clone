...
export function useArrayDocument<T>(
  ...
  useEffect(() => {
    ...
    const fetchData = async (): Promise<void> => {
      try {
        const docsSnapshot = await Promise.all(cachedDocsId.map(id => getDoc(doc(collection, id))))

        const docs = docsSnapshot.filter(doc => doc.exists()).map(doc => doc.data({ serverTimestamps: 'estimate' }))
        ...
        if (includeUser) {
          populateUser(docs as DataWithRef<T>)
        }
