...
export function ThemeContextProvider({ children }: ThemeContextProviderProps) {
  ...
  useEffect(() => {
    const flipTheme = (theme: Theme): Promise<void> | undefined => {
      ...
      if (user) {
        ...
        return updateUserTheme(user.id, { theme })
      }
  ...
  useEffect(() => {
    const flipAccent = (accent: Accent): Promise<void> | undefined => {
      const root = document.documentElement
      ...
      if (user) {
        ...
        return updateUserTheme(user.id, { accent })
      }
  ...
  const changeTheme = ({ target: { value } }: ChangeEvent<HTMLInputElement>): void => setTheme(value as Theme)

  const changeAccent = ({ target: { value } }: ChangeEvent<HTMLInputElement>): void => setAccent(value as Accent)
