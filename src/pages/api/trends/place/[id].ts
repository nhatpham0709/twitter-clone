...
import type { TrendsData, ErrorResponse, TrendsResponse, FilteredTrends } from '@/lib/types/place'
...
export default async function placeIdEndpoint(
  ...
  const response = await fetch(`https://api.twitter.com/1.1/trends/place.json?id=${id}`, AUTH)

  const rawData = (await response.json()) as TrendsData | ErrorResponse

  const data = 'errors' in rawData ? rawData : { trends: rawData[0].trends, location: rawData[0].locations[0].name }
  ...
  if (formattedTrends) {
    const filteredTrends = formattedTrends.filter(({ tweet_volume }) => tweet_volume) as FilteredTrends
  ...
  const formattedData = formattedTrends ? { ...data, trends: formattedTrends } : null
