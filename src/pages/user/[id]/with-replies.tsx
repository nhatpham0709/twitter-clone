...
export default function UserWithReplies() {
  ...
  const { data: pinnedData } = useDocument(doc(tweetsCollection, pinnedTweet ?? 'null'), {
  ...
  const { data: tweets, loading } = useCollection(
    query(tweetsCollection, where('createdBy', '==', id), orderBy('createdAt', 'desc')),
    { includeUser: true }
  )

  return (
    <section>
      <SEO title={`Tweets with replies by ${name as string} (@${username as string}) / Twitter`} />
      {loading ? (
        ...
        <AnimatePresence mode='popLayout'>
          {pinnedData && <Tweet pinned {...pinnedData} key={`pinned-${pinnedData.id}`} />}
          <TweetWithParent data={tweets} />
        </AnimatePresence>
      )}
    </section>
  )
